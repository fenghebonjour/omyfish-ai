...
> **Bite Score's primary weather provider is Visual Crossing (`VISUALCROSSING_API_KEY`), falling back to Open-Meteo (no key needed) and then OpenWeatherMap (`OPENWEATHERMAP_API_KEY`, optional) if earlier providers fail. NOAA tides and local ephem solunar need no keys, and it has no checkpoint dependency. The pure engine tests run fully offline: pytest tests/bite_prediction**
...
| bite_prediction/providers/ | The only I/O boundary -- weather (Visual Crossing, Open-Meteo/OpenWeatherMap fallback), tides (NOAA CO-OPS), solunar (ephem) |
